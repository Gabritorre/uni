...
Nei grafi orientati $E \subseteq V \times V$ cioè è un sottoinsieme di coppie dei nodi
...
cioè un un insieme contenente tutti i possibili sottoinsiemi di 2 elementi di $V$ in cui l'ordine non conta
...
La rappresentazione tramite liste di adiacenza consiste in un array grande quanti sono i nodi, e in ogni cella è presente un puntatore ad una lista linkata contenente gli archi uscenti da quel nodo.
...
Data una matrice non orientata **quadrata**, cioè di dimensione $n \times n$
La traccia si definisce come la somma degli elementi nella diagonale principale.

$$Tr(A) = \sum_{i = 1}^n a_{ii}$$

- $Tr(A) = 0 \hspace{5mm}$ in quanto nella matrice non orientata non ci sono cappi
- $Tr(A^2) = 2|E| \hspace{5mm}$ per il lemma della stretta di mano
- $Tr(A^3) = 6\cdot \text{(numero di triangolo del grafo)}$
	un triangolo è un grafo completo composto da 3 nodi.
...
Si utilizza una matrice $A_G = (a_{ij})$ di grandezza $n \times m$ in cui nelle righe vanno messi i nodi mentre nelle colonne vanno gli archi.
...
Nella definizione di un grafo possiamo aggiungere una informazione in più riguardo al peso di vertici oppure archi, il peso ha un significato dipendente dalla realtà di riferimento.
...
la terza condizione sta a significare che i nodi considerati devono essere connessi da un arco.
...
Il numero di archi in un albero libero è $m-1$, con $m=$ numero di nodi dell'albero.
Il fatto che sia aciclico permette che ogni coppia di nodi venga collegata da un **unico** cammino


## Foresta

Una foresta è un nome per indicare un grafo aciclico, il nome deriva dal fatto che è possibile vedere un grafico aciclico come un insieme di alberi
...
Ad esempio il seguente grafo (nota che si tratta di un singolo grafo) è disconnesso ma possiede 3 componenti connesse: 
