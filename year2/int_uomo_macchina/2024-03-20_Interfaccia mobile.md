...
- progettare l'**architettura dell'informazione** e la **navigazione**:
	cioè una astrazione (schemi e modelli) di come deve essere organizzata l'applicazione, le interazione tra gli utenti dell'app, come cercare una informazione, come raggruppare e organizzare l'informazione.
	c'è una sottile differenza tra:
	- **dato**: cioè fatti grezzi
	- **Conoscenza**: è quello che le persone sanno
	- **informazioni**: una elaborazione dei dati, non sempre restituisce una informazione 
	
	universalmente giusta o sbagliata
...
$$\text{richiesta esplicita -> elaborazione -> risposta esplicita}$$

Questo è un modello problematica in quanto:
...
Gli **schemi** permettono di definire le caratteristiche condivise tra i contenuti in modo da raggrupparli logicamente (classificarli)
...
Schemi di questo tipo sono facili da mantenere e da progettare e da utilizzare, alcuni esempi sono:
...
sono schemi che dividono l'informazione in modo non oggettivo, risulta quindi difficile fare una classificazione, da usare e da mantenere.

Risultano però molto utili nel caso in cui l'utilizzatore sta cercando di raccogliere più materiale possibile su un argomento oppure se l'utilizzatore non sa esattamente cosa sta cercando (cercando una cosa anche largamente correlate riesci ad ottenere quello che cercavi)
...
Viene suddiviso il contenuto in collezioni di processi e funzioni. Schema molto presente nelle app mobile e desktop
...
Le metafore devono essere una cosa conosciuta ma che allo stesso tempo può risultare limitante rispetto al concetto da far capire. Oppure l'utente può esageratamente associare il concetto alla metafora pensando a delle funzioni che in realtà non esistono.
...
le strutture includono: sequenze, gerarchie, ipertesti e modelli orientati ai database.
...
le gerarchia sono presenti nella vita di tutti i giorni (libri, classi sociali, famiglia) e per questo risulta semplice per l'utente capire la struttura dell'informazione e come navigarla.
...
Solitamente non viene utilizzato come metodo principale ma come supplemento ad altri tipi di organizzazione (spesso quello gerarchico)ù
...
Un altra possibilità è organizzare le informazioni su un modello relazionale dei database fatto di tabelle (entità), composte da colonne (attributi) e righe (valori degli attributi).
...
Questo modo di organizzare permette agli utenti di **definire le proprie etichette** che definiscono il contenuto di un dominio di informazioni.
