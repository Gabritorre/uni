...
Implementiamo ad un livello astratto il codice per implementare questo problema utilizzando i processi:
...
Adiamo ad implementare una soluzione *step-by-step* utilizzando i thread.
...
Il problema di questa implementazione è che se il consumatore è più veloce del produttore esso non attende che ci siano dei dati validi nel buffer prima di leggere.

Un approccio potrebbe essere di creare una attesa attiva (*busy waiting*) che il buffer si riempia con dati validi:
...
Questo approccio sistema le cose, però adesso dobbiamo risolvere il fatto che abbiamo assunto di avere un buffer di dimensione illimitata, ciò non è realistico.
...
Per ovviare a questo problema possiamo fermare mettere in attesa il produttore quando i due indici sono a una cella di distanza:
...
Produttore() {
	while(1) {
		// produce il dato 'd'
		while((write_index + 1) % MAX == read_index) {} 	//se la scrittura corrente mi porterebbe allo stesso indice della lettura allora attendo
		buffer[write_index] = d;
		write_index = (write_index + 1) % MAX;
	}
}
...
- spreca tempo di CPU con i cicli a vuoto
- non scala con più produttori e consumatori, in quanto interferirebbero tra loro con gli indici

## Interferenza

Per risolvere il primo problema potremmo pensare di utilizzare una variabile condivisa tra i due thread che si occupa di contare quante celle sono presenti nel buffer e non sono ancora state lette.
...
Infatti è possibile che la variabile counter venga aggiornata contemporaneamente dal consumatore e dal lettore, generando una **race condition** che può portare ad aggiornare la variabile in modo errato:
...
3. salvataggio del nuovo valore in memoria	(viene salvato il valore del registro in memoria)
...
2. Secondo caso:
	...
	Questo accade perche il consumatore ha letto il dato iniziale e poi si è fermato, intanto però il produttore ha aggiornato il valore del `counter`. Quando poi il consumatore riprende l'esecuzione possiede un dato ormai obsoleto
3. Terzo caso (simmetrico al secondo): 
	...
	- Questo caso non è corretto in quanto un incremento e un decremento generano un risultato diverso da quello iniziale.
	Questo accade perche il produttore ha letto il dato iniziale e poi si è fermato, intanto però il consumatore ha aggiornato il valore del `counter`. Quando poi il produttore riprende l'esecuzione possiede un dato ormai obsoleto
