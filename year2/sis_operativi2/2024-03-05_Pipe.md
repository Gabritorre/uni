...
Le pipe sono un canale di comunicazione tra i processi, in altre parole la pipe è un buffer (zona di memoria) in cui un processo scrive dei dati e l'altro processo legge i dati.
...
Cerchiamo di simulare, attraverso un programma, il comportamento della pipe fatta da shell.
...
Consideriamo un processo *lettore* (destinatario) che accetta, su una pipe con nome, messaggi provenienti da più _scrittori_ (mittenti). Gli scrittori mandano 3 messaggi e poi terminano. Quando tutti gli scrittori chiudono la pipe il lettore ottiene 0 come valore di ritorno dalla `read` ed esce.
