...
In caso di trigger per statement e di AFTER trigger per riga la funzione ritorna sempre NULL. Mentre per i BEFORE trigger per riga si ha:
...
creiamo un trigger per gestire l'invariante "Nessun produttore di PC pu`o anche produrre laptop".
