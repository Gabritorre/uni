...
D’altra parte permettono lo scambio di pacchetti tra ogni host in una rete.

Il livello applicazione richiede che i dati:

- arrivino correttamente
- possano essere riordinati dal ricevitore
- duplicati siano rilevati
...
Questo servizio è solitamente usato per applicazioni che accettano la perdita di alcuni dati e non si è interessati a ritrasmetterli.

È importante anche citare l’utilizzo del **numero di porta**: cioè un numero presente sull’header che identifica una determinata applicazione.
...
I **pacchetti** si possono **duplicare**, ma questo può essere dato da dei loop temporanei. Definiamo quindi un **MSL** (Maximum Segment Life): il massimo tempo che un pacchetto e il suo ACK possono persistere nella rete.

Per internet assumiamo che MSL< 120 secondi.

Per identificare i duplicati abbiamo bisogno di avere dei **numeri di sequenza**,

Ci serve quindi una variabile che incrementa nel tempo.
...
infine il mittente manderà un ACK riferito all’ACK ricevuto.
...
2. successivamente la connessione termina
3. il client apre una nuova connessione verso lo stesso server con ISN = $x + \Delta$ che però deve essere calcolata in modulo $n$ per poter non andare all’infinito, supponiamo che accada: ISN = $(x + \Delta)\%n = x$ (lo stesso ISN della vecchia connessione)
4. il client riceve un ACK per il ISN $x$ ma si riferisce alla nuova connessione oppure è una copia di un vecchio ACK?

Se il tempo necessario per ricominciare con i numeri di sequenza è paragonabile all’MSL potrebbe trattarsi di entrambi i casi.
...
![https://i.ibb.co/H2wvZLG/image.png](https://i.ibb.co/H2wvZLG/image.png)
...
4. L’host A riceve l’ACK e conferma che l’host B ha ricevuto correttamente la richiesta iniziale
5. A questo punto per l’host A la connessione è stabilita e il i numeri di sequenza dei segmenti di dati inviati da A inizieranno da x.
...
- `swin`: la dimensione della finestra di chi invia
- `rwin`: la dimensione della finestra di chi riceve
...
![https://i.ibb.co/tY4yS39/image.png](https://i.ibb.co/tY4yS39/image.png)

Se la finestra diventa di dimensione zero, significa che il ricevitore è sovraccarico, in tal caso il mittente smette di mandare e attende gli ACK dal ricevitore.

Periodicamente il mittente rimanda vecchi dati per permettere al ricevitore di mandare rimandare gli ACK nel caso siano stati persi.
...
![https://i.ibb.co/18mnPpZ/image.png](https://i.ibb.co/18mnPpZ/image.png)
...
Questo era un altro esempio sull’importanza di impostare la grandezza dei numeri di sequenza rispetto all’MSL.
...
Diventano però sempre più diffuse reti gigabit non locali in cui il valore di $\text{capacity} \times \text{delay}$ diventa considerevole. In questi casi bisogna aggiornare di conseguenza gli standard.
...
- **Chiusura graduale**: attraverso uno scambio di pacchetti atto a chiudere la connessione in entrambe le direzioni
    
    ![https://i.ibb.co/PWCvB2q/image.png](https://i.ibb.co/PWCvB2q/image.png)
    
- **Chiusura improvvisa**: quando uno dei due host deve chiudere immediatamente la connessione.
    
    ![https://i.ibb.co/TLPg3Cn/image.png](https://i.ibb.co/TLPg3Cn/image.png)
