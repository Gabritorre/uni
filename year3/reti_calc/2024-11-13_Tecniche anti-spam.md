...
Vediamo diverse tecniche utilizzate per combattere lo spam a livello di rete. Si concentriamo su tre protocolli per l'autenticazione delle email: **SPF, DKIM** e **DMARC.**

Con **spam** intendiamo e-mail con contenuti malevoli di vario tipo ad esempio: phishing, malware, pubblicità, truffe…

Se uno spammer è un MUA non è difficile rilevarlo perché per mandare e-mail si deve connettere e autenticarsi ad un MSA e da attraverso di lui mandare tantissime e-mail spam.
...
Gli MTA utilizzano un protocollo chiamato DNSBL (DNS Block List) per verificare se un IP è presente in una blacklist.

**Spamhaus** è una organizzazione la cui attività principale è quella di compilare e tenere aggiornate le blacklist

### Graylist

Un altro fattore importante che gli MTA legittimi aderiscono allo **standard SMTP**, mentre gli spammer spesso no. Gli MTA possono ritardare le risposte ai comandi o restituire errori temporanei per rallentare gli spammer.

Una particolare si utilizza una tecnica chiamata **Graylisting**: gli MTA che utilizzano il graylisting rispondono con un errore `4xx` alla prima connessione da un IP sconosciuto. Gli MTA legittimi riproveranno dopo un po' di tempo, mentre gli spammer tendenzialmente no.
...
Approfondiamo alcuni comandi di SMTP:
...
- `FROM`: specifica l'indirizzo email del mittente visualizzato dal destinatario. Questo è anche l'indirizzo email utilizzato dal destinatario per rispondere all'email.
...
Il Sender Policy Framework (**SPF**) è un protocollo che cerca di contrastare gli spammer impedendogli di utilizzare i comandi `HELO` e `MAIL FROM` (ma non il `FROM`) con un dominio che non possiedono.
...
- controlla se l’indirizzo IP della connessione è uno degli indirizzi permessi dal record SPF
- prende delle decisioni in base al risultato

Nella sintassi dei record SPF ci sono dei **qualificatori** che specificano l'azione da intraprendere quando un indirizzo IP non corrisponde a un meccanismo di autorizzazione:

- `+`: **Pass**, Indica che il mittente è autorizzato a inviare e-mail per quel dominio.
- `-`: **Fail,** Indica che il mittente **non è autorizzato**. Le e-mail da tale IP dovrebbero essere rifiutate.
- `~`: **SoftFail,** Indica che il mittente probabilmente non è autorizzato. Le e-mail da tale IP non vengono contrassegnate come sospette o spam.
- `?`: **Neutral,** Indica che il dominio non dà indicazioni sull'autorizzazione del mittente
...
MSA è l’entità che si occupa dell’autenticazione dell’utente, è l’unica entità che può controllare il campo `FROM` e firmare l’intera email.
...
4. **Verifica della firma**: Quando un MTA ricevente riceve un'email con una firma DKIM, recupera la chiave pubblica del mittente con una risoluzione DNS. L'MTA utilizza quindi la chiave pubblica per verificare la firma digitale e controllare l'integrità del messaggio.
...
DMARC verifica che il dominio nell'intestazione "From" corrisponda al dominio utilizzato o in SPF (quindi con HELO o MAIL FROM) o in DKIM (nel campo `d` dell’header). In caso di mancato allineamento, il server MTA può applicare diverse politiche.
...
Combinando tutti le tecniche che abbiamo visto:
...
![https://i.ibb.co/3mpjwwC/image.png](https://i.ibb.co/3mpjwwC/image.png)
...
4. **Controllo blacklist DNSBL:** Prima di inviare l'email, il MTA di `a.com` controlla l'indirizzo IP `1.2.3.4` non sia in una blacklist tramite una query DNS ad un servizio come *spamhaus*.
5. **Risoluzione DNS inversa:** Il server MTA di `b.com` riceve la connessione dal MTA di `a.com` e effettua una risoluzione DNS inversa sull'indirizzo IP `1.2.3.4` per ottenere il FQDN del MTA mittente.
...
Tutti i metodi che abbiamo visto funzionano fintanto che l’attaccante non riesce modificare la configurazione del server DNS dell’organizzazione in cui è riuscito a bucare.
