...
Ci sono vari modi per creare array, in ogni modo gli array creati possiedono come prototype l’oggetto `Array.prototype` che possiede molti metodi utili per operare sugli array.
...
Può essere usato per fare una **copia superficiale** (shallow copy) degli elementi (cioè se gli elementi sono oggetti copierà solo il riferimento)
...
È importante notare che però non verranno utilizzati come propri indici dell’array, ma solo come proprietà dell’oggetto
...
- `unshift(element)`: aggiunge un elemento all’inizio dell’array e shift gli elementi di una posizione avanti

Contrariamente possiamo rimuovere elementi usando:
...
```jsx
let uppercase = "";
letters.forEach(letter => {    // Note arrow function syntax ere
   uppercase += letter.toUpperCase();
});
uppercase // => "HELLO WORLD"
```

Generalmente le funzioni come `forEach()` invocano la funzione passata come argomento con 3 parametri: il **valore** dell’elemento, **l’indice** dell’elemento e **l’array stesso**.
...
Tali oggetti non hanno come prototipo `Array.prototype` quindi non si possono chiamare i relativi metodi e aspettarsi l’aggiornamento automatico di `length`.
