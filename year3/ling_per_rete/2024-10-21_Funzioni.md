...
Una funzione può accedere ai campi dell’oggetto che ha invocato la funzione con la keyword `this`.
...
- Definizione come **espressioni:** la definizione come espressioni assomiglia alla dichiarazione, ma appaiono all’interno di un'espressione più ampia. Il nome della funzione è opzionale in questo caso.
...
Un'espressione di invocazione consiste nel nome della funzione seguita da parentesi che racchiudono una lista di espressioni che vengono valutate e passate come argomento.
...
In Javascript, le definizioni delle funzioni non specificano un tipo previsto per i parametri, non viene eseguito alcun controllo di tipo sui valori degli argomenti passati e non viene controllato nemmeno il numero di argomenti che vengono passati.
...
I valori di default si possono usare in tutti i modi per definire le funzioni.
...
I parametri rest sono stati introdotti nelle nuove versioni di Javascript. Prima di quella versione del linguaggio, le funzioni che accettavano un numero arbitrario di parametri venivano scritte usando l'oggetto `Arguments`.
...
JavaScript esegue conversioni di tipo secondo le necessità. Quindi, se si scrive una funzione che si aspetta un argomento stringa e poi si chiama quella funzione con un valore di qualche altro tipo, il valore passato verrà semplicemente convertito in una stringa quando dentro la funzione si cerca di usarlo come stringa.
...
- **Assegnate a variabili:** Il nome della funzione diventa quindi solo un riferimento a all’oggetto funzione.
...
Le funzioni JS hanno uno scope lessicale, questo significa che **quando vengono eseguite utilizzano lo scope di quando sono state definite** e non lo scope di quando vengono invocate.
...
Un altro esempio, per una funzione che incrementa un contatore. Al posto di avere il contatore come una proprietà interna alla funzione (che chiunque potrebbe modificare) usiamo una funzione con uno scope privato:
...
In questo caso a `uniqueInteger` è stata assegnata la funzione interna che ha il suo stato privato contenente `counter`. In questo modo la funzione `uniqueInteger` ha accesso esclusivo alla variabile counter.
...
Ad esempio se abbiamo due bottoni e vogliamo che uno incrementi una variabile contatore e l’altro bottone che la decrementi possiamo sfruttare le chiusure per farlo su un variabile condivisa `counter`:
